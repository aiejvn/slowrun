...
def flash_attn_func(q, k, v, causal=False, window_size=(-1, -1)):
    """Attention forward pass. Uses FA3 when available, falls back to PyTorch SDPA. q,k,v: (B, T, H, D)."""
    if _fa3 is not None:
    ...
    v_ = v.transpose(1, 2)
    left = window_size[0]
    # sliding window causal mask when left != -1 and smaller than full sequence
    if causal and left != -1 and left < T:
        rows = torch.arange(T, device=q.device).unsqueeze(1)
        cols = torch.arange(T, device=q.device).unsqueeze(0)
        mask = (cols <= rows) & (cols >= rows - left + 1)
        attn_bias = torch.zeros(T, T, dtype=q.dtype, device=q.device)
        attn_bias[~mask] = float('-inf')
        out = torch.nn.functional.scaled_dot_product_attention(q_, k_, v_, attn_mask=attn_bias)
    else:
        out = torch.nn.functional.scaled_dot_product_attention(q_, k_, v_, is_causal=causal)
...
class GPT(nn.Module):
    ...
    def set_dupe_layers(self, start, end):
        ...
        if start >= n:
            return  # dupe range is beyond this model's depth; silently skip
        end = min(end, n)
        if start >= end:
            return
...
class DistMuonAdamW(torch.optim.Optimizer):
    ...
    def _reduce_adamw(self, group, world_size):
        ...
        if world_size == 1:
            for p in group['params']:
                infos[p] = dict(future=None, grad_slice=p.grad, is_small=True)
    ...
    def _reduce_muon(self, group, world_size):
        ...
        if world_size == 1:
            ...
            return dict(future=None, grad_chunk=grad_chunk, stacked_grads=stacked_grads, chunk_size=chunk_size)
    ...
    def _compute_muon(self, group, info, gather_list, rank):
        ...
        if dist.is_initialized():
            ...
            torch._foreach_copy_(list(params), list(updated[:len(params)].unbind(0)))
...
def train_single_model(model_idx, seed, device, config, autocast_ctx, token_bytes,
    ...
    dupe_device_batch_size = max(1, args.device_batch_size // 2)  # used during dupe for models 1+
...
def main():
    ...
    if _fa3 is not None:
        ...
        print0("Flash Attention 3 not available; falling back to PyTorch SDPA")
